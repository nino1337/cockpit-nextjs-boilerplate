<script type="text/javascript" src="<?php $app->base('lokalize:assets/lokalize.js'); ?>"></script>
<script>

  window.__lokalizeProject = <?php echo  json_encode($project) ; ?>
...
<div>
    <ul class="uk-breadcrumb">
        ...
        <li class="uk-active"><span><?php echo  $project['name'] ; ?></span></li>
    </ul>
</div>

<?php if (isset($project['info']) && $project['info']) { ?>
<div class="uk-margin-large-bottom uk-text-small uk-text-muted">
    <div>
        <?php echo  nl2br(htmlspecialchars($project['info'])) ; ?>
    </div>
</div>
<?php } ?>
...
<div riot-view>
    ...
    <div class="uk-grid uk-flex-middle">
        ...
        <div class="uk-width-1-4">
            <div class="uk-form-icon uk-form uk-text-muted uk-display-block">
                ...
                <input class="uk-form-large uk-form-blank" type="text" ref="txtfilter" placeholder="<?php echo $app("i18n")->get('Filter...'); ?>" onkeyup="{ updatefilter }">
            ...
            <span class="uk-margin-small-right" each="{lang in project.languages}" data-uk-dropdown="mode:'click'">
                ...
                <div class="uk-dropdown">
                    ...
                    <ul class="uk-nav uk-nav-dropdown uk-dropdown-close">
                        ...
                        <li><a onclick="{parent.removeLanguage}"><?php echo $app("i18n")->get('Remove'); ?></a></li>
                    </ul>
                </div>
            </span>

            <span class="uk-margin-small-left" title="<?php echo $app("i18n")->get('Add language'); ?>" data-uk-dropdown="mode:'click'">
        ...
        <div class="uk-flex uk-flex-middle">
            <a class="uk-text-large uk-margin-right uk-form-file" title="<?php echo $app("i18n")->get('Import'); ?>">
            ...
            <a class="uk-text-large uk-margin-right" href="{App.route('/lokalize/export/'+project._id)}" download="{project.name}.csv" title="<?php echo $app("i18n")->get('Export'); ?>"><i class="uk-icon-download"></i></a>
            <a class="uk-button uk-button-primary uk-width-1-1" onclick="{ addKey }"><?php echo $app("i18n")->get('Add Key'); ?></a>
        </div>
    ...
    <div class="uk-width-medium-1-1 uk-viewport-height-1-2 uk-container-center uk-text-center uk-flex uk-flex-middle uk-flex-center" if="{ !App.Utils.count(project.keys) }">
        ...
        <div class="uk-animation-scale uk-text-muted">
            <p>
                <img class="uk-svg-adjust" src="<?php echo $app->pathToUrl('lokalize:icon.svg'); ?>" width="40" height="40" alt="Collections" data-uk-svg />
            </p>
            <span class="uk-text-large"><?php echo $app("i18n")->get('No keys created'); ?></span>
        </div>
    ...
    <div class="uk-panel uk-panel-box uk-panel-card uk-margin lokalize-keys" each="{key in getVisibleKeys() }">
        ...
        <div class="uk-grid">
            ...
            <div class="uk-flex-item-1 uk-form">
                ...
                <div class="uk-flex">
                    ...
                    <div class="uk-flex-item-1">
                        <input class="uk-width-1-1 uk-form-blank uk-text-primary" placeholder="😱 <?php echo $app("i18n")->get('Empty'); ?>" bind="project.values.{parent.project.lang}['{key}']" if="{!parent.project.keys[key].multiline}">

                        <?php if ($app['config/lokalize/multilineMarkdown']) { ?>
                        <cp-field type="markdown" class="uk-width-1-1 uk-display-block" placeholder="😱 <?php echo $app("i18n")->get('Empty'); ?>" bind="project.values.{parent.project.lang}['{key}']" opts="{ {height:150, } }" if="{parent.project.keys[key].multiline}"></cp-field>
                        <?php } else { ?>
                        <textarea class="uk-width-1-1 uk-form-blank uk-text-primary" placeholder="😱 <?php echo $app("i18n")->get('Empty'); ?>" bind="project.values.{parent.project.lang}['{key}']" if="{parent.project.keys[key].multiline}"></textarea>
                        <?php } ?>
                ...
                <div class="uk-flex uk-margin-small-top" each="{lang in parent.project.languages}">
                    ...
                    <div class="uk-flex-item-1">
                        ...
                        <input class="uk-width-1-1 uk-form-blank lang-input" placeholder="😱 <?php echo $app("i18n")->get('Empty'); ?>" bind="project.values.{lang}['{key}']" onfocus="{parent.suggestTranslation}" onblur="{parent.hideTranslation}" lang="{lang}" key="{key}" if="{!parent.project.keys[key].multiline}">
                        <?php if ($app['config/lokalize/multilineMarkdown']) { ?>
                        <cp-field type="markdown" class="uk-width-1-1 uk-display-block" placeholder="😱 <?php echo $app("i18n")->get('Empty'); ?>" bind="project.values.{lang}['{key}']" opts="{ {height:150, placeholder:'😱 <?php echo $app("i18n")->get('Empty'); ?>'} }" if="{parent.project.keys[key].multiline}"></cp-field>
                        <?php } else { ?>
                        <textarea class="uk-width-1-1 uk-form-blank" placeholder="😱 <?php echo $app("i18n")->get('Empty'); ?>" bind="project.values.{lang}['{key}']" onfocus="{parent.suggestTranslation}" onblur="{parent.hideTranslation}" lang="{lang}" key="{key}" if="{parent.project.keys[key].multiline}"></textarea>
                        <?php } ?>
                        ...
                        <div class="lang-suggestion uk-panel-framed uk-margin-small-top uk-margin-small-bottom uk-text-small uk-animation-fade uk-flex" if="{parent.$suggestion.key && parent.$suggestion.key==key && parent.$suggestion.lang==lang && parent.$suggestion.trans !== false }">
                            <strong><?php echo $app("i18n")->get('Suggestion'); ?>:</strong>
            ...
            <div class="uk-margin-small-left">
                <a class="uk-display-block" onclick="{ parent.duplicateKey }" title="<?php echo $app("i18n")->get('Duplicate Key'); ?>"><i class="uk-icon-copy uk-icon-button"></i></a>
                <a class="uk-display-block uk-margin-small-top" onclick="{ parent.removeKey }" title="<?php echo $app("i18n")->get('Delete Key'); ?>"><i class="uk-icon-trash-o uk-icon-button"></i></a>
            </div>
    ...
    <div ref="modalkey" class="uk-modal">
        ...
        <div class="uk-modal-dialog" if="{ $key }">
            ...
            <h3 class="uk-text-bold"><?php echo $app("i18n")->get('Key Editor'); ?></h3>

            <form class="uk-form" onsubmit="{saveKey}">

                <div class="uk-form-row">
                    <label class="uk-text-small"><?php echo $app("i18n")->get('Key'); ?></label>
                    <input class="uk-width-1-1" type="text" placeholder="<?php echo $app("i18n")->get('Key name'); ?>" bind-event="input" bind="$key.name" required>
                </div>

                <div class="uk-form-row">
                    <label class="uk-text-small"><?php echo $app("i18n")->get('Key info'); ?></label>
                    <textarea class="uk-width-1-1 uk-form-large" name="description" placeholder="..." bind-event="input" bind="$key.info" rows="3"></textarea>
                </div>

                <div class="uk-form-row">
                    <field-boolean bind="$key.multiline" label="<?php echo $app("i18n")->get('Multiline'); ?>"></field-boolean>
                </div>
                ...
                <div class="uk-modal-footer uk-text-right">
                    ...
                    <a class="uk-modal-close"><?php echo $app("i18n")->get('Cancel'); ?></a>
                </div>
    ...
    <cp-actionbar>
        <div class="uk-container uk-container-center">
            ...
            <button class="uk-button uk-button-large uk-button-primary" onclick="{ submit }"><?php echo $app("i18n")->get('Save'); ?></button>

            <a class="uk-button uk-button-large uk-button-link" href="<?php $app->route('/lokalize'); ?>">
                <?php echo $app("i18n")->get('Close'); ?>
            </a>
        </div>
    </cp-actionbar>
    ...
    <script type="view/script">
        ...
        LokalizeTranslator.init(<?php echo  json_encode($app->retrieve('lokalize/translationService', null)) ; ?>);
