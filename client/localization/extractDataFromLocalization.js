 ...
 * }
 *
 * First step: filter out every key, where the name contains the localization. In the example above, we receive headline_en, metaTitle_en and metaDescription_en
 * Notice - if no key for the current localization is found, we default it to the german localization
 * Second step: create a new object with keys, that don't have a language reference in them. This brings us the advantage, that we can use the same keys everywhere in our app.
...
const extractDataFromLocalization = (localization, data) => {
  const localizedDataKeys = Object.keys(data).filter(
    (key) => key.substr(key.length - 3) === `_${localization}`
  );

  if (localization === 'de' || localizedDataKeys.length === 0) return data;
